...
public final class EncFSShell {
    ...
    private static List<EncFSFile> getPath(String path) throws IOException {
        ...
        StringTokenizer st = new StringTokenizer(path, EncFSVolume.PATH_SEPARATOR);
        while (st.hasMoreTokens()) {
            ...
            if (curFile.isDirectory()) {
                ...
                if (st.hasMoreTokens()) {
                    throw new FileNotFoundException("'"+pathElement+"' is not a directory!");
                } else {
                    result.add(curFile);
                    found = true;
                }
    ...
    private static boolean createVolume(String path) {
        System.out.print("No EncFS volume found at '"+path+"' would you like to create it? [Yes/No]: ");
        ...
        if (response.equalsIgnoreCase("yes")||response.equalsIgnoreCase("y")) {
            ...
            try {
                ...
                new EncFSVolumeBuilder().withFileProvider(fileProvider).withConfig(config).withPassword(password).writeVolumeConfig();
            } catch (Exception e) {
                e.printStackTrace();
                return false;
            }

            System.out.println("New volume '"+path+"' created successfully.");

            // Open the volume
            try {
                volume = new EncFSVolumeBuilder().withRootPath(path).withPassword(password).buildVolume();
            } catch (Exception e) {
    ...
    public static void main(String[] args) {
        ...
        /*
         * If the given directory or the config file doesn't exist ask for creation.
         */
        ...
        if (!inputDir.exists()||!configFile.exists()) {
            ...
            try {
                volume = new EncFSVolumeBuilder().withRootPath(args[0]).withPassword(password).buildVolume();
            ...
            try {
                ...
                if (curDir==volume.getRootDir()) {
                    ...
                    if (curDir.getParentPath().equals(EncFSVolume.ROOT_PATH)) {
                        System.out.print(EncFSVolume.ROOT_PATH+curDir.getName()+" > ");
                    } else {
                ...
                if ("ls".equals(command)) { // list child directories
                    ...
                    if (pathStr==null) {
                        ...
                        try {
                            ...
                            if (lastPathElement.isDirectory()) {
                                ...
                                System.out.println("'"+pathStr+"'"+" is not a directory!");
                                continue;
                            }
                        } catch (FileNotFoundException e) {
                            System.out.println(e.getMessage());
                    ...
                    Comparator<EncFSFile> comparator = new Comparator<EncFSFile>() {
                        ...
                        public int compare(EncFSFile arg0, EncFSFile arg1) {
                            ...
                            if (options.sortByTime) {
                                long diff = arg0.getLastModified()-arg1.getLastModified();
                                if (diff>0) {
                                    result = -1;
                                } else if (diff==0) {
                                    result = 0;
                    ...
                    for (EncFSFile file : files) {
                        if (options.longListingFormat) {
                            ...
                            if (file.isDirectory()) {
                                System.out.println(file.getName()+EncFSVolume.PATH_SEPARATOR);
                            } else {
                    ...
                    if (command.equals("mkdir")) {
                        try {
                            result = volume.makeDir(EncFSVolume.combinePath(curDir, dirPath));
                        } catch (FileNotFoundException e) {
                            System.out.println(e.getMessage());
                            continue;
                        }
                    ...
                    boolean result;
                    try {
                        result = volume.deletePath(EncFSVolume.combinePath(curDir, filePath), recursive,
                    ...
                    String srcPath = srcPathList.get(srcPathList.size()-1).getPath();
                    ...
                    boolean result;
                    try {
                        result = volume.movePath(srcPath, dstPath, new EncFSShellProgressListener());
                    } catch (IOException e) {
                        System.out.println(e.getMessage());
                        continue;
                    }
                    ...
                    try {
                        srcPathList = getPath(pathArray[0]);
                        /*
                         * If source path is a directory require recursive flag to proceed
                         */
                    ...
                    boolean result;
                    try {
                        result = volume.copyPath(srcPath, dstPath, new EncFSShellProgressListener());
                    } catch (IOException e) {
                        System.out.println(e.getMessage());
                        continue;
                    }
                    ...
                    try {
                        ...
                        System.out.println("Path '"+dirPath+"' doesn't exist!");
                        continue;
                    }
                    ...
                    /*
                     * Current directory goes into the stack also Special handling for absolute paths
                     */
