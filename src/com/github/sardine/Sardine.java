...
public interface Sardine {
     ...
     *
     * @param url
     * Path to the resource including protocol and hostname
     * @return List of resources for this URI including the parent resource itself
     * @throws IOException
     * I/O error or HTTP response validation failure
     */
     ...
     * Path to the resource including protocol and hostname
     * @param depth
     * The depth to look at (use 0 for single ressource, 1 for directory listing)
     * @return List of resources for this URI including the parent resource itself
     * @throws IOException
     * I/O error or HTTP response validation failure
     */
     ...
     * Path to the resource including protocol and hostname
     * @param addProps
     * Properties to add to resource. If a property already exists then its value is replaced.
     ...
     * Properties to add to resource. If a property already exists then its value is replaced.
     * @param removeProps
     * Properties to remove from resource. Specifying the removal of a property that does not exist is not an
     * error.
     * @return The patched resources from the response
     * @throws IOException
     * I/O error or HTTP response validation failure
    ...
    /**
     * Uses <code>PUT</code> to send data to a server with a specific content type header. Repeatable on authentication
     * failure.
     *
     * @param url
     * Path to the resource including protocol and hostname
     ...
     * Input source
     * @param contentType
     * MIME type to add to the HTTP request header
     * @throws IOException
     * I/O error or HTTP response validation failure
     */
    ...
    /**
     * Uses <code>PUT</code> to send data to a server with a specific content type header. Not repeatable on
     ...
     * Enable <code>Expect: continue</code> header for <code>PUT</code> requests.
     * @throws IOException
     * I/O error or HTTP response validation failure
     */
    ...
    /**
     * Uses <code>PUT</code> to send data to a server with specific headers. Not repeatable on authentication failure.
     *
     * @param url
     * Path to the resource including protocol and hostname
     * @param dataStream
    ...
    /**
     * Put an exclusive write lock on this resource. A write lock must prevent a principal without the lock from
     * successfully executing a PUT, POST, PROPPATCH, LOCK, UNLOCK, MOVE, DELETE, or MKCOL on the locked resource. All
     * other current methods, GET in particular, function independently of the lock.
     * <p/>
     * A WebDAV compliant server is not required to support locking in any form. If the server does support locking it
     * may choose to support any combination of exclusive and shared locks for any access types.
     *
     * @param url
     * Path to the resource including protocol and hostname
     * @return The lock token to unlock this resource. A lock token is a type of state token, represented as a URI,
     * which identifies a particular lock. A lock token is returned by every successful <code>LOCK</code>
     * operation in the lockdiscovery property in the response body, and can also be found through lock
     ...
     * Path to the resource including protocol and hostname
     * @param token
     * The lock token to unlock this resource.
     * @throws IOException
     * I/O error or HTTP response validation failure
     * @see #lock(String)
     */
     ...
     *
     * @param hostname
     * The hostname to enable preemptive authentication for.
     */
    ...
    /**
     * Releasing any resources that might be held open. This is an optional method, and callers are not expected to call
     * it, but can if they want to explicitly release any open resources. Once a client has been shutdown, it should not
